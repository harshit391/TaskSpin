...
export function WeeklySchedule() {
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          />
          <p className="text-(--text-muted)">Generating schedule...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="py-12">
        <div className="card border-(--error)/30 bg-(--error)/5 text-center max-w-lg mx-auto">
          <svg
            width="32"
            height="32"
            viewBox="0 0 24 24"
          ...
          <h3 className="heading-display heading-sub text-(--error)">
            Schedule Error
          </h3>
  ...
  if (tasks.length === 0 && !hasActivePoolSubtasks) {
    return (
      <div className="text-center py-20">
        ...
        <h2 className="heading-display heading-section text-(--text-secondary)">
          No Schedule Yet
        </h2>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <select
            ...
            className="px-3 py-2 text-sm font-medium rounded-lg border border-(--border-color) bg-(--bg-secondary) text-foreground cursor-pointer focus:outline-none focus:border-accent"
          >
      ...
      {oneTimeTasks.length > 0 && todayDay && (
        <motion.div
          ...
          className="card border-accent/30 bg-linear-to-r from-accent/5 to-transparent"
        >
      ...
      {pendingSideTaskCount > 0 && (
        <div className={`flex items-center gap-3 px-4 py-3 rounded border ${
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
            className={overdueSideTaskCount > 0 ? 'text-(--error)' : 'text-(--text-muted)'}>
            <path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2" />
            <rect x="8" y="2" width="8" height="4" rx="1" ry="1" />
          </svg>
          <span className="text-sm text-(--text-secondary) flex-1">
            You have <strong className={overdueSideTaskCount > 0 ? 'text-(--error)' : 'text-foreground'}>{pendingSideTaskCount} side task{pendingSideTaskCount !== 1 ? 's' : ''}</strong> pending
            {overdueSideTaskCount > 0 && (
              <span className="text-(--error)"> ({overdueSideTaskCount} overdue)</span>
            )}
          </span>
        </div>
      )}
