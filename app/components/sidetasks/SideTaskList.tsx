...
export function SideTaskList() {
  ...
  const [movingTask, setMovingTask] = useState<SideTask | null>(null);
  ...
  const getDueLabel = (dueDate?: string) => {
    ...
    if (dueDate < today) return { text: 'Overdue', cls: 'text-(--error)' };
    if (dueDate === today) return { text: 'Due today', cls: 'text-(--warning)' };
    const due = new Date(dueDate + 'T00:00:00');
    const diff = Math.ceil((due.getTime() - new Date(today + 'T00:00:00').getTime()) / (1000 * 60 * 60 * 24));
    if (diff <= 3) return { text: `Due in ${diff}d`, cls: 'text-(--warning)' };
    return { text: `Due ${due.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}`, cls: 'text-(--text-muted)' };
  };

  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="animate-pulse text-(--text-muted)">Loading side tasks...</div>
      </div>
    );
  }
          ...
          >
            ...
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                {errors.link && <p className="mt-1 text-sm text-(--error)">{errors.link.message}</p>}
              </div>
              <div className="w-44">
      ...
      <AnimatePresence mode="popLayout">
        {pending.length === 0 && completed.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-16">
            ...
            <h3 className="heading-display heading-sub text-(--text-secondary)">No Side Tasks</h3>
          ...
          <div className="space-y-2">
            {pending.map((task, index) => {
              ...
              if (editingId === task.id) {
                return (
                  <div key={task.id} className="card border-accent space-y-3">
                    ...
                    <div className="flex justify-end gap-2">
                      <button onClick={() => setEditingId(null)} className="px-3 py-1.5 text-sm text-(--text-muted) hover:text-foreground">Cancel</button>
                      <button onClick={saveEdit} className="px-3 py-1.5 text-sm bg-accent text-white rounded hover:bg-(--accent)/80">Save</button>
                    </div>
                  </div>
                );
              }
              ...
              return (
                <motion.div
                  ...
                  className="group flex items-start gap-3 p-3 rounded border border-(--border-color) bg-(--bg-secondary) hover:border-accent transition-colors"
                >
                  {/* Checkbox */}
                  <button
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button onClick={() => startEdit(task)} className="p-1.5 text-(--text-muted) hover:text-accent rounded" title="Edit">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                      </svg>
                    </button>
                    <button onClick={() => setMovingTask(task)} className="p-1.5 text-(--text-muted) hover:text-accent rounded" title="Move to Main Tasks">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M15 3h6v6" />
                        <path d="M10 14L21 3" />
                        <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
                    ...
                    <button onClick={() => deleteSideTask(task.id)} className="p-1.5 text-(--text-muted) hover:text-(--error) rounded" title="Delete">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {showCompleted && (
              ...
              >
                {completed.map((task) => (
                  <div
                    key={task.id}
                    className="group flex items-start gap-3 p-3 rounded border border-(--border-color) bg-(--bg-secondary) opacity-50"
                  >
                    <button
                      onClick={() => toggleComplete(task.id)}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-(--text-muted) line-through">{task.name}</p>
                    </div>
                    <button onClick={() => deleteSideTask(task.id)} className="p-1.5 text-(--text-muted) hover:text-(--error) rounded opacity-0 group-hover:opacity-100 transition-opacity" title="Delete">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <polyline points="3 6 5 6 21 6" />
                        <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                      </svg>
